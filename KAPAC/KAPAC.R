...
option_list <- list(
  ...
  make_option(c("--create_files_for_each_motif"), default=FALSE, action="store", type="logical", help="If set, for each kmer detailed files (activities, errors, z-scores) will be created (NOTE: dependent on the sitecount matrix (--sitecount_matrix) and the selected k-mers (--selected_motifs), thousands of directories and files might be created)."),
  make_option(c("--row_center_expression"), default=TRUE, action="store", type="logical", help="If set, the expression matrix (--expression_matrix) will be row-centered."),
  make_option(c("--treat_minus_ctrl"), default=FALSE, action="store", type="logical", help="If true, plot treatment minus control."),
