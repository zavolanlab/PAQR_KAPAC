...
parser.add_argument("--samples",
                    dest="samples",
                    nargs="*",
                    help="ordered list of names for the samples in the input file")
    ...
    with open( options.rel_usages, "r") as relUseIn:
        for line in relUseIn:
            ...
            if n == 1:
                # process first line
                # consistency checks:
                # check number of files in the relative usage file
                # and compare it with the number of provided samples
                if (len(F) - 10) != len(options.samples):
                    syserr("[ERROR] Number of samples provided is inconsistent with number of samples in input file.\n")
                    syserr("[ERROR] Input file contains info for %i samples\n" % (len(F) - 10) )
                    sys.exit(2)
