...
def main():
	...
	parser.add_option("-n","--sample-size",action="store",type="int",dest="sample_size",default=100,help="Number of equal-spaced nucleotide positions picked from mRNA. Note: if this number is larger than the length of mRNA (L), it will be halved until it's smaller than L. default=%default")
        ...
        parser.add_option("--names",dest="sample_names",action="store",type="string",help="sample names, comma separated (no spaces allowed); number must match the number of provided bam_files")
	...
	for f_idx in range(len(bamfiles)):
        ...
        for ex in sample_TINS_per_transcript:
                print >>sys.stdout, "%s\t%s" % (ex, "\t".join(map(str, sample_TINS_per_transcript[ex]) ) )
